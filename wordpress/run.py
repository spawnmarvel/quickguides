...
class ApiWorker():
    ...
    def get_all_categories(self):
        ...
        dict_of_categories = {}
        try:
            response = requests.get("https://mellowthinking.com//wp-json/wp/v2/categories?per_page=50", auth=(self.user, self.cred))
    ...
    def create_category(self):
        try:
            cat_url = "https://mellowthinking.com/wp-json/wp/v2/categories"
            names = ["Notech", "Youtech", "Theytech"] # "RDoxxon", "RDRINCO", "Runder Mifflin", "Rupiter Mining Corporation", "Ragruato", "Rrmacham", "Rnitech" , "Rontoso", "Retacortex", "Rekall"]
            for n in names:
                print(n)
                data = {
                "description": "cat for customer autogenerated",
                "name": n,
                "slug": n.lower(),  
                "parent": 21, # 21 Customers
                 }
                # Send the HTTP request
                response = requests.post(cat_url, auth=(self.user, self.cred), json=data)
                # Check the response
                if response.status_code == 201:
                    logging.info("Categories created successfully")
                else:
                    logging.error("Failed to create Categories: " + response.text)
        except Exception as ex:
            logging.error(ex)
    ...
    def get_all_posts(self):
        ...
        try:
            ...
            for t in tmp_li:
                ...
                if post_title.lower() == "rekall":
                    print("We already have:", post_title, " in Customer category")
