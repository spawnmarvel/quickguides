...
### We now have a client that trust the server and uses the server CA certificates, verifies server, checks that the server present a certificate with our configured SNI in CN of the sertificate. Shovel is now configured with AMQPS, SSL/TLS towards the server. This can be enough in many situations.
