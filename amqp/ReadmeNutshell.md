...
## Install in general and configure for tls or mtls
...
When the tnc is success for TcpTestSucceeded or true, we can configure:

8. Configure a tls shovel (this is not what we are doing in this tutorial)
...
8. Configure a mtls shovel

A tls shovel is configured like this for easy troubleshooting.

* VM1 configure shovel certificates and a ca bundle with VM2 root/intermediate certificates.
* VM2 configure tls 5671 for external and 5672 for internal connections
* VM2 configure
...
9. On VM1 make the queues as auto-create in the shovel configuration, if the shovel is running then GOTO 10

---
10. On VM2 queues should now be autogenerated

---

11. Check shovel management on VM1-> the status should be running
...
#### advanced.config mtls example ip :frog:
...
#### advanced.config mtls example DNS :frog:
...
#### rabbitmq.conf mtls example :frog:
...
The hosts file (or DNS) ensures the client can resolve the server's hostname to the correct IP address. Mutual TLS (mTLS) then verifies both the server's and the client's identities using trusted X.509 certificates. Together, this provides a strong, standards-based security model that protects against unauthorized access and helps prevent man-in-the-middle attacks.
